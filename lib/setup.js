...
module.exports = async function() {
	try {
		...
		log(`2. Verify that your models and migrations are correct`, "info");
		log(`3. Inculde these variables in your environment: MONGODB_URI, DATABASE_URI and SESSION_SECRET`, "info");
		log(`4. Run "knex migrate:latest" to rub your migrations`, "info");
		log(`5. Configure webpack.js file`, "info");
		log(`6. Run "git init" if you want version management control`, "info");
		log(`7. Start the server with "npm start"`, "info");
	} catch (err) {
		log(err, "error");
	}
};
	...
	return new Promise(async function(resolve, reject) {
		try {
			...
			for (const graphRelation of modelGraph) {
				...
				// console.log(graphRelation.sourceModel.modelName);
