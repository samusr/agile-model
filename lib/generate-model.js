...
const createMigrationFiles = (/**@type {ImplementationRelation} */ relation) => {
	return new Promise(async function(resolve, reject) {
		try {
			...
			const migrationFilePath = `${rootDir}${moment().format("YYYYMMDDHHmmss")}_create_${relation.sourceModel.tablename}_table.js`;
			...
			log("Migration File Created!", "info");
			return resolve();
		} catch (err) {
			return reject(err);
		}
...
const createDBServiceFiles = (/**@type {ImplementationRelation} */ relation) => {
	return new Promise(async function(resolve, reject) {
		try {
			...
			log("DB files created successfully", "info");
			return resolve();
		} catch (err) {
			return reject(err);
		}
