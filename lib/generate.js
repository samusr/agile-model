...
module.exports = async function(modelname, args) {
    try {
        let modelName = _.capitalize(modelname[0]) + _.camelCase(modelname.replace("-", "_")).substring(1);
        let modelFileName = `${modelname}.js`.toLowerCase();
        ...
        tableName = tableName.replace("-", "_").toLowerCase();
